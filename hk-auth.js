...
window.HKAuth = (function () {
  ...
  function buildCallback() {
    const next = nextParam() || 'index.html';
    ...
    // Absolute callback URL (Supabase requires absolute), resolved against the current page so it
    // works at a domain root (forge.hackknow.com/login.html) AND a project subpath
    // (gaganchauhan1997.github.io/yahavi-forge/login.html).
    return new URL(CALLBACK_PATH, location.href).href;
  }

  function consumeNext() {
    let n = 'index.html';
    try { n = sessionStorage.getItem('hk-next') || 'index.html'; sessionStorage.removeItem('hk-next'); } catch (_) {}
    // Open-redirect guard: block absolute URLs (scheme) and protocol-relative; allow same-origin paths.
    if (n.startsWith('//') || /^[a-z][a-z0-9+.-]*:/i.test(n)) n = 'index.html';
    return n;
  }
