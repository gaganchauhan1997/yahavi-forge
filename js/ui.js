...
(function (root) {
  ...
  /* ─────── sidebar ─────── */
  // Category icons — maps group id → emoji
  const CAT_ICONS = {
    build:    '🔨',
    analyze:  '🔍',
    tailor:   '✂️',
  ...
  function renderSidebar() {
    ...
    const groups = F.SIDEBAR.map((g) => {
      if (g.id === 'home') {
        return `<button class="nav-tab" data-tool="home" style="font-family:var(--font-body);font-weight:700;font-size:13px;">🏠 Home</button>`
      }
      const items = g.items
        .map((id) => {
          const t = F.TOOLS[id]
          if (!t) return ''
          return `<button class="nav-tab" data-tool="${escAttr(id)}" title="${escAttr(t.subtitle)}">${escText(t.title)}</button>`
        })
        .join('')
      const icon = CAT_ICONS[g.id] || '▸'
      return `<div class="sidebar-cat" data-cat="${escAttr(g.id)}">
        <button class="sidebar-cat-toggle" type="button" data-cat-toggle="${escAttr(g.id)}" aria-expanded="false">
          <span class="cat-ico">${icon}</span>
          <span class="cat-label">${escText(g.label)}</span>
    ...
    // Wire collapsible category toggles
    ...
    // Auto-open BUILD category by default (first one always visible)
    const buildCat = nav.querySelector('.sidebar-cat[data-cat="build"]')
    if (buildCat) {
      buildCat.classList.add('open')
      const toggle = buildCat.querySelector('[data-cat-toggle]')
